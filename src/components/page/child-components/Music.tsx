...
const Music = memo((props: { lyricInfo: ILyric; musicInfo: IMusicInfo }) => {
  // 获取音乐信息相关
  const { singers, name: songName } = props.musicInfo
  // 获取歌词相关信息
  const { currentLyricIndex, lyricList, lyricBoxRef } = props.lyricInfo
  return (
    <aside w='300px' flex='~ col' items='center' className='<lg:(hidden)'>
